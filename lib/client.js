...
function ClientInterface(socket) {
  ...
  this._socket.once('finish', this._resourceEnded.bind(this));
...
ClientInterface.prototype.log = function (details, callback) {
  if (this._closeing) throw new Error('the socket is closeing');
...
ClientInterface.prototype._resourceEnded = function (callback) {
  this._closeing = true;
  ...
  if (this.isReading()) {
    ...
    if (callback) this._reader.once('close', callback);
  }
...
ClientInterface.prototype._resourceClosed = function () {
  // this method may already have been called, in that case both isReading and
  // isWriteing should return false, so the method woun't do anything.
  this._resourceEnded(this.emit.bind(this, 'close'));
};
